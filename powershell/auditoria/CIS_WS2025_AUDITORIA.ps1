...
# SCRIPT DE AUDITORÍA COMPLETO - Valida todos los controles Level 1 MS
#
...
Write-Host " ProjectDB - IF5100 UCR" -ForegroundColor Cyan
Write-Host "========================================================" -ForegroundColor Cyan
Write-Host ""
...
function Check-UserRight {
    ...
    try {
        ...
        if ($null -eq $line -or $line -match "^$Privilege\s*=$") {
            if ($ExpectedSIDs -eq "") {
                Write-Host "[PASS] CIS $CIS - $Desc = No One" -ForegroundColor Green; $script:pass++
            } else {
                Write-Host "[FAIL] CIS $CIS - $Desc (esperado: $ExpectedSIDs, actual: vacío)" -ForegroundColor Red; $script:fail++
            }
        } else {
...
# =============================================================================
# SECCIÓN 18 - ADMINISTRATIVE TEMPLATES (muestra)
# =============================================================================
Write-Host "--- SECCIÓN 18: Administrative Templates ---" -ForegroundColor Magenta
...
Skip "18.9.5.1" "Turn on VBS = Enabled" "Requiere hardware enterprise (Hyper-V/UEFI específico)"
...
Check-Reg "HKLM:\SOFTWARE\Policies\Microsoft\Windows\System" "AllowCustomSSPsAPs" 0 "18.9.26.1" "Allow Custom SSPs = Disabled"
Skip "18.9.26.2" "LSASS RunAsPPL = Enabled" "Puede causar boot failure en Azure VM sin Secure Boot configurado"
Check-Reg "HKLM:\SOFTWARE\Policies\Microsoft\Windows\System" "BlockUserFromShowingAccountDetailsOnSignin" 1 "18.9.27.1" "Block account details on sign-in = Enabled"
Check-Reg "HKLM:\SOFTWARE\Policies\Microsoft\Windows\System" "DontDisplayNetworkSelectionUI" 1 "18.9.27.2" "Do not display network selection UI = Enabled"
Check-Reg "HKLM:\SOFTWARE\Policies\Microsoft\Windows\System" "DontEnumerateConnectedUsers" 1 "18.9.27.3" "Do not enumerate connected users = Enabled"
Check-Reg "HKLM:\SOFTWARE\Policies\Microsoft\Windows\System" "EnumerateLocalUsers" 0 "18.9.27.4" "Enumerate local users = Disabled"
Check-Reg "HKLM:\SOFTWARE\Policies\Microsoft\Windows\System" "DisableLockScreenAppNotifications" 1 "18.9.27.5" "Turn off lock screen notifications = Enabled"
...
Check-Reg "HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender" "PUAProtection" 1 "18.9.65.8" "PUA Protection = Enabled"
...
Check-Reg "HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\Terminal Services" "fDisableCdm" 1 "18.9.75.2" "Do not allow drive redirection = Enabled"
...
Check-Reg "HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\Rpc" "EnableAuthEpResolution" 1 "18.9.36.1" "RPC Endpoint Mapper Auth = Enabled"
Skip "18.9.36.2" "Restrict Unauthenticated RPC clients = Authenticated" "Puede bloquear conexiones RDP remotas"
Skip "18.9.25.1" "LAPS: Configure backup directory = Active Directory" "Sin Active Directory en VM standalone"
Write-Host ""
...
Write-Host "========================================================" -ForegroundColor Cyan
Write-Host " RESUMEN AUDITORÍA CIS WS2025 - ProjectDB IF5100" -ForegroundColor Cyan
Write-Host "========================================================" -ForegroundColor Cyan
